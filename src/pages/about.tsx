...
const history: [string, string, boolean][] = [
  ['1995',
   'Partha Pratim Roy leaves an earlier business and builds Prarthana in Santoshpur, a small residential building on a plot belonging to a family who decided to develop rather than sell. Eight flats over three floors. It sets the pattern for everything after it.',
   false],
  ['2000s',
...
const commitments: [string, string, string][] = [
  ...
  ['04', 'We are still reachable after handover',
   'The office has not moved since 1995 and the buildings are all within a few kilometres of it.'],
];
    ...
    <>
      ...
      <Section eyebrow="01 / The name" tone="light">
        <div className="mt-10 grid gap-12 lg:grid-cols-[1fr_1.6fr] lg:gap-20">
          <div />
          <Reveal delay={1}>
            <DisplayHeading className="max-w-4xl text-[clamp(2.8rem,5.4vw,5.8rem)] text-[hsl(var(--primary))]">
              Ashima
            </DisplayHeading>
      ...
      <Section eyebrow="03 / Thirty years" tone="dark">
        <div className="mt-2 grid gap-9 lg:grid-cols-[.8fr_1.8fr]">
          <div />
          <Reveal>
            <DisplayHeading className="max-w-4xl text-[clamp(2.8rem,5.6vw,6rem)]" em="years.">
              Thirty
            </DisplayHeading>
          </Reveal>
        </div>
          ...
          {history.map(([label, text, draft], i) => (
            <Reveal key={label} delay={(i % 4) as 0 | 1 | 2 | 3}>
              <div className="grid gap-5 border-b border-[hsl(var(--card))]/20 py-8 md:grid-cols-[9rem_1fr] md:gap-10 md:py-10">
                <div className="font-display text-4xl italic text-[hsl(var(--secondary))]">{label}</div>
                <div>
                  <p className="max-w-3xl text-base leading-7 text-[hsl(var(--card))]/85">{text}</p>
                  {draft && (
                    <p className="gap-note">
      ...
      <Section eyebrow="04 / How we work" tone="light">
        <div className="mt-2 grid gap-9 lg:grid-cols-[.8fr_1.8fr]">
          <div />
          <Reveal>
            <DisplayHeading className="max-w-4xl text-[clamp(2.6rem,5.4vw,5.6rem)] text-[hsl(var(--primary))]"
                            em="Solid ground.">
              Plain words.
            </DisplayHeading>
            <p className="mt-8 max-w-xl text-base leading-7 text-[hsl(var(--muted-foreground))]">
          ...
          {commitments.map(([number, title, body], i) => (
            <Reveal key={number} delay={(i % 4) as 0 | 1 | 2 | 3}>
              <div className="grid gap-5 border-b border-[hsl(var(--primary))]/15 py-7 md:grid-cols-[80px_1.05fr_1fr] md:items-start md:gap-8 md:py-9">
                <span className="font-display text-3xl italic text-[hsl(var(--accent))]"
                      data-testid={`text-commitment-number-${number}`}>{number}</span>
                <h3 className="max-w-md font-display text-3xl leading-[.95] text-[hsl(var(--primary))]"
        ...
        <div className="mt-10 grid gap-16 lg:grid-cols-[.88fr_1.12fr] lg:items-center lg:gap-24">
          <Reveal className="relative">
            ...
            <div className="absolute -bottom-9 -right-4 hidden bg-[hsl(var(--accent))] p-6 text-[hsl(var(--card))] lg:block">
              ...
              <div className="mt-2 font-ui text-[9px] uppercase tracking-[0.15em]">projects delivered</div>
            </div>
          </Reveal>

          <Reveal delay={1}>
            <DisplayHeading className="text-[clamp(3rem,5.6vw,6.4rem)] !leading-[.87] text-[hsl(var(--primary))]"
                            em="Roy.">
              Partha<br />Pratim
            </DisplayHeading>
            <blockquote className="mt-10 border-l-2 border-[hsl(var(--accent))] pl-6">
              <p className="max-w-md font-display text-2xl leading-tight text-[hsl(var(--primary))]">
                A building is honest or it isn&rsquo;t, and you can only tell by looking
                at the parts nobody photographs.
              </p>
              <cite className="mt-4 block font-ui text-[10px] uppercase not-italic tracking-[0.16em] text-[hsl(var(--muted-foreground))]">
